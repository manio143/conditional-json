...
let ParserRules = [
  ...
  { name: 'alternative', symbols: ['conjunctive'], postprocess: id },
  {
    ...
    postprocess: ([exprL, _, op, __, exprR]) => ({ type: 'logical', op: op.text, left: exprL, right: exprR }),
  },
  ...
  {
    name: 'comparison',
    symbols: ['valueExpression', '_', lexer.has('comparer') ? { type: 'comparer' } : comparer, '_', 'valueExpression'],
    postprocess: ([exprL, _, op, __, exprR]) => ({ type: 'comparison', op: op.text, left: exprL, right: exprR }),
  },
  ...
  },
  {
    ...
    postprocess: ([fun, _, __, args]) => ({ type: 'application', fun: fun.value, args: args }),
  },
  { name: 'argList', symbols: [], postprocess: () => [] },
  { name: 'argList', symbols: ['expression'], postprocess: ([expr]) => [expr] },
  {
    name: 'argList',
  ...
  },
  {
    ...
    postprocess: ([_, __, identifier]) => ({ type: 'accessor', path: [identifier] }),
  },
  {
    name: 'accessor',
    symbols: ['accessor', lexer.has('dot') ? { type: 'dot' } : dot, 'identifier'],
    postprocess: ([accessor, __, identifier]) => ({ type: 'accessor', path: [...accessor.path, identifier] }),
  },
  {
    name: 'accessor',
    symbols: [
    ...
    postprocess: ([accessor, _, __, expr]) => ({ type: 'accessor', path: [...accessor.path, expr] }),
  },
  ...
  },
  {
    ...
    postprocess: ([number]) => ({ type: 'number', value: Number.parseFloat(number) }),
  },
  {
    name: 'string',
  ...
  },
  {
    ...
    postprocess: ([bool]) => ({ type: 'bool', value: bool.value == 'true' }),
  },
