...
const evaluate_ = (
	...
	switch (expr.type) {
		...
		case 'bool':
			return expr;
		case 'accessor': {
			const evalutedPath = expr.path.map((e) =>
				evaluate_(e, functions, context),
			);
		...
		case 'application': {
			if (Object.hasOwn(specialFunctions, expr.fun)) {
				return specialFunctions[expr.fun]((e) =>
					evaluate_(e, functions, context),
				).apply({}, expr.args);
			}
			const func = functions[expr.fun];
			if (func === undefined)
				throw `The function '${expr.fun}' was not found.`;
		...
		case 'logical': {
			const left = evaluate_(expr.left, functions, context);
			if (left.type !== 'bool')
				throw `Expression evaluated to value '${left.value}', but expected a boolean.`;
			// lazy evaluation, return fast if left implies the result
			if (expr.op === '||' && left.value)
				return { type: 'bool', value: true };
			if (expr.op === '&&' && !left.value)
				return { type: 'bool', value: false };
			const right = evaluate_(expr.right, functions, context);
			if (right.type !== 'bool')
				throw `Expression evaluated to value '${right.value}', but expected a boolean.`;
			if (expr.op === '||')
				return { type: 'bool', value: left.value || right.value };
			if (expr.op === '&&')
				return { type: 'bool', value: left.value && right.value };
...
const builtInFunctions: FunctionStore = {
	date: (strExpr, ...rest) => {
		if (!strExpr)
			throw `Function 'date' expects a single string argument, received zero arguments.`;
	...
	not: (boolExpr, ...rest) => {
		if (!boolExpr)
			throw `Function 'not' expects a single bool argument, received zero arguments.`;
...
const specialFunctions: {
	...
	) => (...args: Expression[]) => SimpleExpression;
} = {
	iff:
		(evaluate: (e: Expression) => SimpleExpression) =>
		(
			...
			if (rest.length > 0)
				throw `Function 'iff' expects 3 arguments, got ${3 + rest.length}.`;
